...
func execForHangAction(uid string, ctx context.Context, expModel *spec.ExpModel, pid int32, args string) *spec.Response {
	...
	if cgroups.Mode() == cgroups.Unified {
		if result := runCommandCgroupV2(cgroupRoot, int(pid), command); result != "" {
			return spec.ReturnFail(spec.OsCmdExecFailed, result)
		}
	} else {
		if result := runCommandCgroupV1(cgroupRoot, int(pid), command); result != "" {
			return spec.ReturnFail(spec.OsCmdExecFailed, result)
		}
	}
...
func runCommandCgroupV1(cgroupRoot string, pid int, command *exec.Cmd) string {
	control, err := cgroups.Load(osexec.Hierarchy(cgroupRoot), osexec.PidPath(pid))
	if err != nil {
		return fmt.Sprintf("cgroups load failed, %s", err.Error())
	}

	if err := command.Start(); err != nil {
		return fmt.Sprintf("command start failed, %s", err.Error())
	}

	// add target cgroups
	if err = control.Add(cgroups.Process{Pid: command.Process.Pid}); err != nil {
		if err := command.Process.Kill(); err != nil {
			return fmt.Sprintf("create experiment failed, %v", err)
		}
	}
	return ""
}

func runCommandCgroupV2(cgroupRoot string, pid int, command *exec.Cmd) string {
	group, err := cgroupsv2.PidGroupPath(pid)
	if err != nil {
		return fmt.Sprintf("cgroups load failed, %s", err.Error())
	}
	control, err := cgroupsv2.LoadManager(cgroupRoot, group)
	...
	// add target cgroups
	if err = control.AddProc(uint64(command.Process.Pid)); err != nil {
		if err := command.Process.Kill(); err != nil {
			return fmt.Sprintf("create experiment failed, %v", err)
		}
	}
